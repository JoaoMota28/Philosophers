...
/*   Updated: 2025/09/01 23:26:52 by jomanuel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	int			i;
	long long	philo_left_time;
	long long	philo_right_time;
	...
	while (++i < data->philo_num)
	{
		...
		if (i == 0)
			philo_left_time = data->philosophers[data->philo_num - 1].last_eat_time;
		else
			philo_left_time = data->philosophers[i - 1].last_eat_time;
		if (i == data->philo_num - 1)
			philo_right_time = data->philosophers[0].last_eat_time;
		else
			philo_right_time = data->philosophers[i + 1].last_eat_time;
		if (philo_time <= philo_left_time && philo_time <= philo_right_time)
...
static int	manager_check_death(t_data *data)
{
	...
	{
		pthread_mutex_lock(&data->manager);
		if (get_time_ms() - data->philosophers[i].last_eat_time > data->time_to_die)
		{
			pthread_mutex_unlock(&data->manager);
